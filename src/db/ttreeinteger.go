...
func(p *tnodeInteger) Search(searchValue Integer) []ROWNUM{
  ...
  node := p
  for ; ; {
    if node.leftNode != nil && searchValue < node.values[0].key  {
      node = node.leftNode
      continue
    }
    if node.rightNode != nil && searchValue > node.values[node.dataCount-1].key {
      node = node.rightNode
      continue
    }
    break
  }
  isMatch,pos := binarySearchInteger(node.values,searchValue,0,node.dataCount-1)
  if isMatch == true {
